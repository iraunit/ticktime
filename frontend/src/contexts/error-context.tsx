...
export function ErrorProvider({children}: ErrorProviderProps) {
    ...
    const showToast = useCallback((error: ApiError, options?: { persist?: boolean }) => {
        ...
        const errorMessages = error.message.split(';').map((msg: string) => msg.trim()).filter(Boolean);

        if (isNetworkError || isServerError) {
            // For network/server errors, show the first message with retry action
            toast.error(errorMessages[0] || error.message, {
            ...
            // For validation errors, show each message as a separate toast
