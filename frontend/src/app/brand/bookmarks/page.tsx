...
export default function BrandBookmarksPage() {
    ...
    const fetchBookmarks = useCallback(async () => {
        ...
        try {
            ...
            console.log('Bookmarks data:', response.data.bookmarks);
        ...
        <div className="min-h-screen">
            <div className="container mx-auto px-4 py-4 max-w-7xl">
                ...
                <div className="relative mb-6">
                    <div
                        className="absolute inset-0 bg-gradient-to-r from-pink-500/5 via-purple-500/5 to-indigo-500/5 rounded-xl -m-2"></div>
                        ...
                        {filteredBookmarks.map((bookmark) => (
                                        ...
                                        <div className="flex items-start gap-4 flex-1">
                                            {bookmark.influencer?.profile_image ? (
                                                <div className="w-16 h-16 rounded-full overflow-hidden shadow-md">
                                                    ...
                                                    />
                                                    <div
                                                        className="w-16 h-16 bg-gradient-to-r from-pink-500 to-purple-600 rounded-full flex items-center justify-center shadow-md hidden">
                            <span className="text-xl font-bold text-white">
                              {bookmark.influencer?.name?.charAt(0) || '?'}
                                                ...
                                                <div
                                                    className="w-16 h-16 bg-gradient-to-r from-pink-500 to-purple-600 rounded-full flex items-center justify-center shadow-md">
                          <span className="text-xl font-bold text-white">
                            {bookmark.influencer?.name?.charAt(0) || '?'}
                          </span>
                                                </div>
                          ...
                          <span className="text-sm text-gray-600">
                            ...
                            <HiHeart className="w-4 h-4 inline mr-1"/>
                                                        {(bookmark.influencer.engagement_rate || 0).toFixed(1)}% engagement
                          </span>
                                                    <span className="text-sm text-gray-600">
                            {bookmark.influencer.minimum_collaboration_amount && bookmark.influencer.minimum_collaboration_amount > 0
                                ? `Min: ${formatCurrency(bookmark.influencer.minimum_collaboration_amount)}`
                                : bookmark.influencer.rate_per_post && bookmark.influencer.rate_per_post > 0
                                    ...
                                    : 'Rate not specified'
                                                        ...
                                                        if (config) {
                                                            ...
                                                            return (
                                                                <Badge
                                                                    key={platform}
                                                                    variant="outline"
                                                                    className={`text-xs ${config.bg} ${config.border} ${config.color} flex items-center gap-1`}
                                                                >
                                                ...
                                                <div className="flex gap-2 mb-3">
                                                    {(bookmark.influencer.categories || []).map((category) => (
                                                        <Badge key={category}
                                                               className="bg-pink-100 text-pink-800 border-0 text-xs">
                                                            {category}
                                                        </Badge>
                                                    ))}
                                                </div>
                                                ...
                                                <div className="space-y-2">
                                                    {/* Collaboration Types */}
                                                    {bookmark.influencer.collaboration_types && bookmark.influencer.collaboration_types.length > 0 && (
                                                        <div className="flex gap-2 flex-wrap">
                                                            {bookmark.influencer.collaboration_types.map((type) => {
                                                    ...
                                                    <div className="flex gap-2 flex-wrap">
                                                        {bookmark.influencer.barter_ready && (
                                                            <Badge variant="outline"
                                                                   className="text-xs bg-orange-50 text-orange-700 border-orange-200">
                                                                Barter Ready
                                                            </Badge>
                                                        )}
                                                        {bookmark.influencer.commerce_ready && (
                                                            <Badge variant="outline"
                                        ...
                                        <div className="lg:w-80">
                                            <div className="bg-gray-50 rounded-lg p-4">
                                                ...
                                                <div className="mt-3 pt-3 border-t border-gray-200">
                                                    ...
                                                    {bookmark.last_collaboration && (
                                                        <p className="text-xs text-gray-500">
                                                            Last
                                                            collaboration: {formatDate(bookmark.last_collaboration)}
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex gap-2 mt-4">
                                                ...
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => window.open(`/influencer/${bookmark.influencer.id}`, '_blank')}
                                                    className="flex-1"
                                                >
