...
export function UnifiedSidebar({ userType }: UnifiedSidebarProps) {
  ...
  const [logoData, setLogoData] = useState<{ logo: string | null; name: string }>({
    logo: null,
    name: "TickTime"
  });
    ...
    const fetchUserData = async () => {
      try {
        if (userType === 'brand') {
          ...
          // For influencers, use profile data
  ...
  return (
    <>
      {/* Mobile Navbar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-[60] bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            {logoData.logo && userType === 'influencer' ? (
              ...
              <div className="h-8 w-8 bg-gradient-to-br from-red-500 via-orange-500 to-red-600 rounded-full flex items-center justify-center">
          ...
          <div className="border-t border-gray-200 p-4 bg-gradient-to-br from-gray-50 via-white to-gray-50">
            <div className="relative">
              ...
              <button
                ...
                className="w-full flex items-center p-2 rounded-xl hover:bg-gray-100 transition-all duration-200 group"
              >
                <div className="relative group flex-shrink-0">
                  {logoData.logo && userType === 'influencer' ? (
                    <OptimizedAvatar 
                ...
                {isExpanded && (
                  <div className="ml-3 animate-in slide-in-from-left-2 duration-300 flex-1 text-left">
                    <p className="text-sm font-semibold text-gray-900 truncate">
                      {userType === 'brand' ? 'Brand Account' : `${user?.first_name || ''} ${user?.last_name || ''}`.trim() || 'User'}
                    </p>
                ...
                {isExpanded && (
                  <HiChevronDown className={`h-4 w-4 text-gray-400 transition-transform duration-200 ${isProfileMenuOpen ? 'rotate-180' : ''}`} />
                )}
              </button>

              {/* Profile Dropdown Menu */}
              {isProfileMenuOpen && isExpanded && (
                <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-gray-200 rounded-lg shadow-lg py-1 animate-in slide-in-from-bottom-2 duration-200">
                  <Link
              ...
              {!isExpanded && (
                <div className="mt-2">
                  <button
                    onClick={handleLogout}
                    className="w-full flex items-center justify-center p-2 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-200"
                    title="Sign Out"
                  >
